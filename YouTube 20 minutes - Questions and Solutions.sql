/*SQL Questions / Answers using SQL code.*/

/*YouTube video: https://www.youtube.com/watch?v=30W5wjgJR08*/
/* Code from file: https://github.com/WebDevSimplified/Learn-SQL/blob/master/README.md*/
...
/* TASK 1 - Create songs table.*/
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/1.sql*/
CREATE TABLE songs (
  id INT NOT NULL AUTO_INCREMENT,
  name VARCHAR(255) NOT NULL,
  length FLOAT NOT NULL,
  album_id INT NOT NULL,
  PRIMARY KEY (id), -- Primary key is id column
  FOREIGN KEY (album_id) REFERENCES albums(id) -- Create foreign key with refernec to albums table id column.
);




/*Run below code in a new file-- TO ADD all data.
 https://github.com/WebDevSimplified/Learn-SQL/blob/master/README.md
...
/* TASK 2 - Select only the Names of all the Bands.*/
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/2.sql*/
SELECT name AS 'Band Name' -- Change name of column from name to Band Name
FROM Bands;




/* TASK 3 - Select the Oldest Album.*/
...
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/3.sql*/

SELECT * FROM albums -- select albums table 
WHERE release_year IS NOT NULL -- only release year trhat have a vaue
ORDER BY release_year -- order by release year oldest to newest
LIMIT 1; -- Show 1st result only



/*BONUS TASK 3B:*/ 
/* TASK 3 - Select the Newest Album.*/
/* Make sure to only return one result from this query, and that you are not returning any albums that do not have a release year.*/
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/3.sql*/

SELECT * FROM albums -- select albums table 
WHERE release_year IS NOT NULL -- only release year trhat have a vaue
ORDER BY release_year DESC -- order by release year oldest to newest
LIMIT 1; -- Show 1st result only
...
/*INNER JOIN*/
/* TASK 4 - Get all Bands that have Albums.*/
/*Gets all the different albumns that have a band id in bands table is same as  album table band id.*/
/*https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/4.sql*/

/* This assummes all bands have a unique name */
SELECT DISTINCT bands.name AS 'Band Name' -- Show distinct results only not duplicate names.
FROM bands
...
/*TASK 5.*/
/*LEFT JOIN*/
/* TASK 5 - Get all Bands that have No Albums.*/
/*This is very similar to #4 but will require more than just a join.
Return the band name as Band Name.*/
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/5.sql*/

/* THIS CODE DID NOT WORK FOR THE PAGE THEY PROVIDED. SEE MEXT PARAGRAPH WHERE A SUBSCRIBER LEFT THE CORRECT CODE
SELECT bands.name AS 'Band Name'
FROM bands
LEFT JOIN albums 
ON bands.id = albums.band_id
GROUP BY albums.band_id -- Group band name by band id
HAVING COUNT(albums.band_id) = 0; -- count band id which have 0 / no albums. */



/*TASK 5 --- CORRECT CODE. */
/*YouTube video: https://www.youtube.com/watch?v=30W5wjgJR08*/
/* CORRECT CODE FROM SUBSCRIBER.*/
SELECT b.name AS 'Band Name' -- Added Alias
FROM bands AS b -- Added Alias
LEFT JOIN albums AS a ON b.id = a.band_id -- Added Alias
GROUP BY b.id  -- Group band name by band id 
HAVING COUNT(a.id) = 0;  -- count band id which have 0 / no albums.




/*INNER JOIN & SUM AGGREGATE FUNCTION.*/
/* TASK 6 - Get the Longest Album.*/
...
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/6.sql*/
...
GROUP BY songs.album_id -- Group by albumn ID
ORDER BY Duration DESC -- List showing longest 1st.
LIMIT 1; -- Only show 1st result



/* 6B. EXTRA: Show all albums inorder of longest 1st.*/
SELECT
  albums.name as Name,
  albums.release_year as 'Release Year',
  SUM(songs.length) as 'Duration'
...
/* TASK 7 - Update the Release Year of the Album with no Release Year.*/
...
/* https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/7.sql*/

/* This is the query used to get the id, which is id 4*/
SELECT * 
FROM albums 
where release_year IS NULL;

/*Update release year to 1986 for id 4.*/
UPDATE albums
...
/* TASK 8 - Insert a record for your favorite Band and one of their Albums.
If you performed this correctly you should be able to now see that band and album in your tables.
https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/8.sql*/

INSERT INTO bands (name) -- Create new bands row with name Haken in bands table
VALUES ('Haken');

/* This is the query used to get the band id of the band just added */
  SELECT id 
  FROM bands -- Order band by id 
  ORDER BY id DESC LIMIT 1; -- return highest band id only

INSERT INTO albums (name, release_year, band_id) -- Create new row in albums table
VALUES ('Vector', 2018, 8);

/* This is the query used to view the albums id of the band just added into the table. */
  SELECT * 
  FROM albums
  ORDER BY id DESC LIMIT 1; -- return highest band id only
...
/* TASK 9 - Delete the Band and Album you added in #8. Above we 1st created band and then 2nd the album.
SO must delete albumn 1st and then 2nd band.
The order of how you delete the records is important since album has a foreign key to band.
https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/9.sql*/
...
/*Calculates AVERAGE.*/
/* TASK 10 - Get the Average Length of all Songs.
Calculates the average length of songs in the songs table.
https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/10.sql*/
...
/*Calculates MAXIMUM and does JOIN.*/
/* TASK 11 - Select the longest Song off each Album.
Calculates the average length of songs in the songs table.
Return the album name as Album, the album release year as Release Year, and the longest song length as Duration.
For each album shows song with longest song duration grouped by album id.
https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/11.sql*/

SELECT
  albums.name AS 'Album',
  albums.release_year AS 'Release Year',
  MAX(songs.length) AS 'Duration' -- Calculates maximum song length
FROM albums
JOIN songs ON albums.id = songs.album_id -- Join where album.albums.id = songs.songs.id
GROUP BY songs.album_id; -- Shows longest song for each of our different albumns
...
/*Multiple JOINS.*/
/* TASK 12 - Get the number of Songs for each Band.
...
https://github.com/WebDevSimplified/Learn-SQL/blob/master/solutions/12.sql*/

SELECT
  bands.name AS 'Band', -- will appear in results table
  COUNT(songs.id) AS 'Number of Songs' -- will appear in results table
FROM bands -- data is from bands table
JOIN albums ON bands.id = albums.band_id -- 1st Join, join band id with album id
JOIN songs ON albums.id = songs.album_id -- 2nd Join, join albumn id with song id
GROUP BY albums.band_id; -- then group by band id in albumn table
/*So if 1 album for band 3 has 4 songs 
and another album for band 3 has 3 songs 
in for count the total band 3 has 7 songs.*/




/*12B) Above reults but then sorted / ordered by number of songs*/
SELECT
  bands.name AS 'Band', -- will appear in results table
  COUNT(songs.id) AS 'Number of Songs' -- will appear in results table
FROM bands -- data is from bands table
...
and another album for band 3 has 3 songs 
in for count the total band 3 has 7 songs.*/
ORDER BY 'Number of Songs' DESC; -- then order table results by number of songs
